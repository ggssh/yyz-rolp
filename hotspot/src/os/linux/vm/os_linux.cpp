...
void os::dump_thread_majflt_and_cputime() {
  ...
  for (JavaThread* jt = Threads::first(); jt != NULL; jt = jt->next()) {
    ...
    proc_majflt_and_cputime(proc_name, &majflt, &user_time, &sys_time);
    gclog_or_tty->print_cr("JavaThread %s(tid=%d), Majflt=%ld, user=%ldms, sys=%ldms",
      jt->name(), tid, majflt, user_time, sys_time);
  }

  // NonJavaThread
  NonJavaThreads njts;
  njts.do_thread(VMThread::vm_thread());
  Universe::heap()->gc_threads_do(&njts);
  WatcherThread *wt = WatcherThread::watcher_thread();
  if (wt != NULL) njts.do_thread(wt);

  for(NonJavaThread* njt = njts.first(); njt != NULL; njt = njt->get_next()){
    tid = njt->get_thread()->osthread()->thread_id();
    snprintf(proc_name, 64, "/proc/self/task/%d/stat", tid);
    proc_majflt_and_cputime(proc_name, &majflt, &user_time, &sys_time);
    gclog_or_tty->print_cr("NonJavaThread %s(tid=%d), Majflt=%ld, user=%ldms, sys=%ldms",
      njt->get_thread()->name(), tid, majflt, user_time, sys_time);
  }
}
...
void os::dump_javathread_majflt() {
  ...
  for(JavaThread* jt = Threads::first(); jt != NULL; jt = jt->next()){
    ...
    snprintf(proc_name, 64, "/proc/self/task/%d/stat", tid);
    gclog_or_tty->print_cr("JavaThread %s, Majflt=%ld", jt->name(), proc_majflt(proc_name));
  }
}

void os::dump_non_javathread_majflt() {
  pid_t tid;
  char proc_name[64];
  NonJavaThreads njts;
  ...
  for(NonJavaThread* njt = njts.first(); njt != NULL; njt = njt->get_next()){
    tid = njt->get_thread()->osthread()->thread_id();
    snprintf(proc_name, 64, "/proc/self/task/%d/stat", tid);
    gclog_or_tty->print_cr("NonJavaThread %s, Majflt=%ld", njt->get_thread()->name(), proc_majflt(proc_name));
  }
}
